...
class MonitoringPage(QWidget):
    ...
    def __init__(self, user_info: dict, db: DatabaseManager, input_config=None, model_config=None, **kwargs):
        ...
        if input_config is None:
            ...
            self.input_type = input_config['type']  # 'camera', 'rtsp' or 'file'
            ...
            if self.input_type == 'camera':
                ...
                self.video_path = None
                self.rtsp_url = input_config['rtsp_url']
                print(f"[INFO] RTSP 선택됨: {self.rtsp_url[:60]}...")
            else:
                self.camera_index = None
        ...
        # 모델: 임베드 시 .env USE_MODEL, 없으면 다이얼로그
        ...
        self.fall_alert_threshold = 30  # 초
        ...
        try:
            ...
            if os.path.exists(model_path):
                ...
                if hasattr(self.rf_model, 'verbose'):
                ...
                if hasattr(self.rf_model, 'feature_names_in_'):
                    ...
                    with open(feature_path, "r") as f:
                        ...
                        for line in lines[2:]:
                            if ". " in line:
                                self.feature_columns.append(line.strip().split(". ", 1)[1])
                print(f"✅ 낙상 감지 모델 로드: {model_path}")
            else:
                print(f"⚠️ RF 모델 없음: {model_path}")
        except Exception as e:
            print(f"⚠️ RF 로드 실패: {e}")
            import traceback
            traceback.print_exc()
    ...
    def start_monitoring(self):
        ...
        try:
            if self.input_type == 'camera':
                ...
                # === RTSP 스트림 모드 (rtsp_player3.py 참고: CAP_FFMPEG + 33ms 타이머) ===
                self.add_log(f"[INFO] RTSP 연결 시도...")
                QApplication.processEvents()
                self.cap = cv2.VideoCapture(self.rtsp_url, cv2.CAP_FFMPEG)
                
                if not self.cap.isOpened():
                    self.add_log(f"❌ RTSP 스트림을 열 수 없습니다. URL·네트워크를 확인하세요.")
                    return
                
                self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
                self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
                
                self.add_log(f"✅ RTSP 연결 성공 (OpenCV FFMPEG)")
                
                timer_interval = 33  # rtsp_player3과 동일: 33ms (~30 FPS)
                self.video_control_panel.setVisible(False)
            ...
            if self.input_type == 'camera':
                ...
                self.status_label.setText('📡 RTSP Active')
            else:
    ...
    def update_frame(self):
        ...
        try:
            ...
            # ⭐ 프레임 읽기 실패 처리
            if not ret:
                if self.input_type == 'camera' or self.input_type == 'rtsp':
                    if self.frame_count % 100 == 0:
                        self.safe_add_log("[WARN] 프레임 읽기 실패 (실시간 스트림)")
                    return
    ...
    def on_switch_input(self):
        ...
        backup_rtsp = getattr(self, 'rtsp_url', None)
        
        if backup_type == 'camera':
            current_source = f"Camera {backup_camera}"
        elif backup_type == 'rtsp':
            current_source = f"RTSP: {backup_rtsp[:50]}..." if backup_rtsp else "RTSP"
        else:
            current_source = f"File: {os.path.basename(backup_path)}"
        ...
        if self.input_type == 'camera':
            ...
            self.video_path = None
            self.rtsp_url = input_config['rtsp_url']
            self.add_log(f"[SUCCESS] Input changed to: RTSP")
        else:  # file
            self.camera_index = None
            self.video_path = input_config['filepath']
            self.rtsp_url = None
    ...
    def init_stgcn_model(self):
        ...
        if not STGCN_AVAILABLE:
            ...
            return False
        model_path = getattr(self, 'stgcn_model_path', None)
        if not model_path or not os.path.exists(model_path):
            fallback = os.path.join(_GUI_DIR, "checkpoints_finetuned", "best_model_finetuned.pth")
            model_path = fallback if os.path.exists(fallback) else None
        if not model_path:
            self.add_log("[ERROR] ST-GCN 모델 경로를 찾을 수 없습니다.")
            return False
        try:
        ...
        if len(self.keypoints_buffer) >= self.stgcn_buffer_size:
            ...
            try:
                ...
                if label == 'Fall':
                    ...
                    if elapsed >= self.fall_alert_threshold and not self.fall_alert_sent:
                        self.safe_add_log(f"[EMERGENCY] ⚠️ {elapsed:.0f}초간 낙상 지속 — 의식 상실 의심!")
                        self.save_event_to_db('Emergency', confidence)
                        self.fall_alert_sent = True
                        self.show_emergency_popup(elapsed)
    ...
    def show_emergency_popup(self, elapsed):
        """비모달 긴급 알림 팝업"""
        if self.emergency_dialog is not None:
            return
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle("🚨 긴급 알림")
        dlg.setFixedSize(400, 200)
        dlg.setStyleSheet("background-color: #fff0f0;")
        
        layout = QVBoxLayout(dlg)
        
        title = QLabel("🚨 낙상 지속 감지!")
        title.setStyleSheet("color: #e74c3c; font-size: 20px; font-weight: bold;")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(title)
        
        msg = QLabel(f"낙상 상태 {elapsed:.0f}초 지속\n의식 상실이 의심됩니다.\n즉시 확인이 필요합니다.")
        msg.setStyleSheet("color: #333; font-size: 14px;")
        msg.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(msg)
        
        btn = QPushButton("확인")
        btn.setStyleSheet("background-color: #e74c3c; color: white; padding: 8px; font-weight: bold; border-radius: 5px;")
        btn.clicked.connect(lambda: self.close_emergency_popup())
        layout.addWidget(btn)
        
        dlg.setModal(False)
        dlg.show()
        self.emergency_dialog = dlg

    def close_emergency_popup(self):
        """긴급 알림 팝업 닫기"""
        if self.emergency_dialog is not None:
            self.emergency_dialog.close()
            self.emergency_dialog = None