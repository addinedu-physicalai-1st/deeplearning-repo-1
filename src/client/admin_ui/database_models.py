...
클라이언트 .env(ADMIN_UI_ENV_DIR) 또는 admin_ui/.env의 DB_* 사용
"""
...
def _load_db_config_from_env() -> dict:
    """통합 환경: client .env(ADMIN_UI_ENV_DIR)의 DB_* 사용. 없으면 admin_ui/.env 사용."""
    base = os.environ.get("ADMIN_UI_ENV_DIR") or _GUI_DIR
    # 통합 환경(ADMIN_UI_ENV_DIR 설정 시): env_config.get_db_config로 client .env 기준 동일 로직 사용
    if os.environ.get("ADMIN_UI_ENV_DIR"):
        try:
            import sys
            # client/admin_ui/database_models.py -> 상위 2단계 = 프로젝트 루트
            _project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            if _project_root not in sys.path:
                sys.path.insert(0, _project_root)
            from env_config import get_db_config
            cfg = get_db_config(base_dir=base)
            return {
                "host": cfg["host"],
                "port": cfg["port"],
                "user": cfg["user"],
            ...
            pass  # env_config 없으면 아래 기본 로직으로
    ...
    return {
        ...
        "database": os.environ.get("DB_NAME", "home_safe_user"),
    }


class DatabaseManager:
    """데이터베이스 연결 및 관리 (.env DB_* 사용)"""
