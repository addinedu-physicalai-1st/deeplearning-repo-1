...
-- 1) 기존 DB 삭제 (외래키 무시하고 삭제)
SET FOREIGN_KEY_CHECKS = 0;
DROP DATABASE IF EXISTS home_safe;
SET FOREIGN_KEY_CHECKS = 1;

-- 2) 계정 생성 (이미 있으면 삭제 후 재생성)
DROP USER IF EXISTS 'homesafe'@'localhost';
CREATE USER 'homesafe'@'localhost' IDENTIFIED BY 'homesafe1234!';

-- 3) DB 생성 및 권한 부여
CREATE DATABASE home_safe CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
GRANT ALL PRIVILEGES ON home_safe.* TO 'homesafe'@'localhost';
FLUSH PRIVILEGES;
...
-- 완료
-- ============================================================
SELECT 'home_safe DB 및 homesafe 계정 설정 완료.' AS Status;
SELECT User, Host FROM mysql.user WHERE User = 'homesafe';
