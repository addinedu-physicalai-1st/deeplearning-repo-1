...
class InputSourceDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 설명
        desc = QLabel("실시간 카메라, RTSP 스트림 또는 동영상 파일을 선택하세요.")
        ...
        # RTSP 라디오 버튼
        self.radio_rtsp = QRadioButton("📡 RTSP 스트림")
        self.radio_rtsp.setFont(QFont('Arial', 12, QFont.Weight.Bold))
        self.radio_rtsp.setStyleSheet("padding: 5px;")
        type_group.addButton(self.radio_rtsp)
        layout.addWidget(self.radio_rtsp)
        
        # RTSP URL 패널
        self.rtsp_panel = self.create_rtsp_panel()
        layout.addWidget(self.rtsp_panel)
        ...
        self.radio_rtsp.toggled.connect(self.on_type_changed)
    ...
    def create_rtsp_panel(self):
        """RTSP URL 입력 패널 생성"""
        panel = QGroupBox("RTSP URL")
        panel_layout = QVBoxLayout()
        
        # URL 입력
        url_layout = QHBoxLayout()
        self.rtsp_url_edit = QLineEdit()
        self.rtsp_url_edit.setPlaceholderText("예: rtsp://192.168.1.100:554/stream 또는 rtsp://admin:password@ip:554/path")
        self.rtsp_url_edit.setMinimumHeight(36)
        url_layout.addWidget(self.rtsp_url_edit)
        
        panel_layout.addLayout(url_layout)
        
        # 안내 문구
        hint = QLabel("RTSP 스트림 URL을 입력하세요. 네트워크 카메라/스트리밍 장치의 주소입니다.")
        hint.setStyleSheet("color: #7f8c8d; padding: 5px; font-size: 11px;")
    ...
    def on_type_changed(self):
        ...
        is_rtsp = self.radio_rtsp.isChecked()
        is_file = self.radio_file.isChecked()
        
        self.camera_panel.setEnabled(is_camera)
        self.rtsp_panel.setEnabled(is_rtsp)
        self.rtsp_url_edit.setEnabled(is_rtsp)
        self.file_panel.setEnabled(is_file)
    ...
    def on_accept(self):
        ...
        if self.radio_camera.isChecked():
        ...
        elif self.radio_rtsp.isChecked():
            # RTSP 스트림
            rtsp_url = self.rtsp_url_edit.text().strip()
            if not rtsp_url:
                from PyQt6.QtWidgets import QMessageBox
                QMessageBox.warning(self, "URL 입력", 
                                   "RTSP URL을 입력해주세요.")
                return
            if not rtsp_url.lower().startswith("rtsp://"):
                from PyQt6.QtWidgets import QMessageBox
                QMessageBox.warning(self, "URL 형식", 
                                   "RTSP URL은 rtsp:// 로 시작해야 합니다.")
                return
            ...
            print(f"[INFO] RTSP 선택됨: {rtsp_url[:50]}...")
...
def show_input_selection_dialog(parent=None):
    ...
    Returns:
        dict: {
            'type': 'camera' or 'rtsp' or 'file',
            'camera_index': int (카메라인 경우),
            'rtsp_url': str (RTSP인 경우),
            'filepath': str (파일인 경우)
        }
        또는 None (취소 시)
    """
