...
class STGCNInference:
    ...
    def __init__(self, 
                 model_path: str = str(
                     Path(__file__).resolve().parent / "st_gcn" / "checkpoints_v2" / "best_model.pth"
                 ),
                 device: str = 'auto',
...
def test_basic():
    ...
    # 모델 로드 (gui_ver4/st_gcn/checkpoints_v2/best_model.pth 기준)
    model_path = Path(__file__).resolve().parent / "st_gcn" / "checkpoints_v2" / "best_model.pth"
    
    try:
        inference = STGCNInference(model_path=str(model_path))
